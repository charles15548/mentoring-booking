...
export async function POST(request: Request) {
  try {
    ...
    if (!body.businessId || !body.startDateTime || !body.endDateTime)
    ...
    const result = await graphRequest(
      `/solutions/bookingBusinesses/${businessId}/getStaffAvailability`,
      {
        method: "POST",
        body: JSON.stringify({
          staffIds: body.staffIds ?? [],
          startDateTime: {
            dateTime: toGraphUtcDateTime(body.startDateTime),
            timeZone: "UTC",
          },
          endDateTime: {
            dateTime: toGraphUtcDateTime(body.endDateTime),
            timeZone: "UTC",
          },
        }),
      },
