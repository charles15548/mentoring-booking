...
export default function ReservasPage() {
  ...
  useEffect(() => {
    void (async () => {
      ...
      if (!response.ok) setError("No se pudieron cargar tus reservas.");
      else setItems((await response.json()) as Reservation[]);
      setLoading(false);
    })();
  }, []);
  return (
    <div className="page-content">
      <section className="welcome-row">
        <div>
          ...
          <h1>Mis reservas</h1>
          <p className="intro">
            Consulta las sesiones que has agendado con tus mentores.
          </p>
        </div>
      </section>
      {loading ? (
        ...
        <div className="empty-state">
          ...
          <p>Cuando confirmes una sesión aparecerá aquí.</p>
        </div>
      ) : (
        <section className="reservation-list">
          {items.map((item) => (
            <article className="reservation-card" key={item.id}>
              <div>
                <p className="eyebrow">{item.status || "confirmada"}</p>
                <h2>{item.serviceName || "Sesión de mentoría"}</h2>
                <p>{item.customerName || "Reserva PROUNI"}</p>
              </div>
              <time>
                {item.startDateTime
